...
def test_get_slots_in_week_by_booking_empty(test_admin):
    booking_slots = get_slots_from_db_for_schedule(test_admin.get_id(), "2021-03-02", "booking", 7)
    assert booking_slots == []


def test_get_slots_in_week_by_booking(test_admin):
    ...
    booking_slots = get_slots_from_db_for_schedule(test_admin.get_id(), "2021-02-03", "booking", 7)
    assert booking_slots == json


def test_get_slots_in_week_by_available_empty(test_admin):
    available_slots = get_slots_from_db_for_schedule(test_admin.get_id(), "2021-02-03", "available", 7)
    assert available_slots == []


def test_get_slots_in_week_by_available(test_admin):
    ...
    booking_slots = get_slots_from_db_for_schedule(test_admin.get_id(), "2021-02-03", "available", 7)
    assert booking_slots == json
