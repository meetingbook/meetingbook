...
def test_get_slots_in_day_empty(test_admin):
    slots = get_slots_from_db_for_schedule(test_admin.get_id(), "2021-03-02", filter=None, days=1)
    assert slots == []


def test_get_slots_in_day(test_admin):
    ...
    booking_slots = get_slots_from_db_for_schedule(test_admin.get_id(), "2021-02-03", filter=None, days=1)
