...
def send_email(admin_id, request_body, mail, link_id):
    msg_admin = send_admin(admin_id, request_body, link_id)
    msg_guest = send_guest(request_body, link_id)
    mail.send(msg_admin)
    mail.send(msg_guest)


def send_guest(request_body, link_id):
    msg_guest = Message('Hello, you a booked time',
                        sender='booking@meetingbook.com',
                        recipients=[request_body['guest_email']])
    msg_guest.body = '''Hey you booked the time: {start}-{end}.Canel booking: http://localhost:5000/calendars/{link_id}/bookings/{booking_id} '''.format(
...
def send_admin(admin_id, request_body, link_id):
